If you cannot find the answer to your question here, and you have read Primer and Advanced\+Guide, send it to \href{mailto:googletestframework@googlegroups.com}{\texttt{ googletestframework@googlegroups.\+com}}.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md740}{}\doxysubsection{Why should I use Google Test instead of my favorite C++ testing framework?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md740}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md741}{}\doxysubsection{I\textquotesingle{}m getting warnings when compiling Google Test.  Would you fix them?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md741}
...
If you see warnings when compiling Google Test, we suggest that you use the {\ttfamily -\/isystem} flag (assuming your are using G\+CC) to mark Google Test headers as system headers. That\textquotesingle{}ll suppress warnings from Google Test headers.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md742}{}\doxysubsection{Why should not test case names and test names contain underscore?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md742}
...
If you violate the rule, there may not be immediately consequences, but your test may (just may) break with a new compiler (or a new version of the compiler you are using) or with a new version of Google Test. Therefore it\textquotesingle{}s best to follow the rule.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md743}{}\doxysubsection{Why is it not recommended to install a pre-\/compiled copy of Google Test (for example, into /usr/local)?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md743}
...
In order to prevent build output files from overwriting one another, you\textquotesingle{}ll need to change the {\ttfamily Intermediate Directory} settings for the newly created platform configuration across all the projects. To do this, multi-\/select (e.\+g. using shift-\/click) all projects (but not the solution) in the {\ttfamily Solution Explorer}. Right-\/click one of them and select {\ttfamily Properties}. In the left pane, select {\ttfamily Configuration Properties}, and from the {\ttfamily Configuration} dropdown, select {\ttfamily All Configurations}. Make sure the selected platform is {\ttfamily x64}. For the {\ttfamily Intermediate Directory} setting, change the value from {\ttfamily \textbackslash{}} to {\ttfamily \textbackslash{}}. Click {\ttfamily OK} and then build the solution. When the build is complete, the 64-\/bit binaries will be in the {\ttfamily msvc\textbackslash{}x64\textbackslash{}Debug} directory.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md745}{}\doxysubsection{Can I use Google Test on Min\+G\+W?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md745}
...
Please contact {\ttfamily googletestframework@googlegroups.\+com} if you are interested in improving the support for Min\+GW.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md746}{}\doxysubsection{Why does Google Test support E\+X\+P\+E\+C\+T\+\_\+\+E\+Q(\+N\+U\+L\+L, ptr) and A\+S\+S\+E\+R\+T\+\_\+\+E\+Q(\+N\+U\+L\+L, ptr) but not E\+X\+P\+E\+C\+T\+\_\+\+N\+E(\+N\+U\+L\+L, ptr) and A\+S\+S\+E\+R\+T\+\_\+\+N\+E(\+N\+U\+L\+L, ptr)?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md746}
...
Finally, with the growth of Google \mbox{\hyperlink{class_mock}{Mock}}\textquotesingle{}s \href{../../googlemock/docs/CookBook.md\#using-matchers-in-google-test-assertions}{\texttt{ matcher}} library, we are encouraging people to use the unified {\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+T\+H\+A\+T(value, matcher)} syntax more often in tests. One significant advantage of the matcher approach is that matchers can be easily combined to form new matchers, while the {\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+NE}, etc, macros cannot be easily combined. Therefore we want to invest more in the matchers than in the {\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+X\+X()} macros.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md747}{}\doxysubsection{Does Google Test support running tests in parallel?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md747}
Test runners tend to be tightly coupled with the build/test environment, and Google Test doesn\textquotesingle{}t try to solve the problem of running tests in parallel. Instead, we tried to make Google Test work nicely with test runners. For example, Google Test\textquotesingle{}s X\+ML report contains the time spent on each test, and its {\ttfamily gtest\+\_\+list\+\_\+tests} and {\ttfamily gtest\+\_\+filter} flags can be used for splitting the execution of test methods into multiple processes. These functionalities can help the test runner run the tests in parallel.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md748}{}\doxysubsection{Why don\textquotesingle{}t Google Test run the tests in different threads to speed things up?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md748}
...
If you think about it, it\textquotesingle{}s already hard to make your code work when you know what other threads are doing. It\textquotesingle{}s much harder, and sometimes even impossible, to make your code work when you don\textquotesingle{}t know what other threads are doing (remember that test methods can be added, deleted, or modified after your test was written). If you want to run the tests in parallel, you\textquotesingle{}d better run them in different processes.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md749}{}\doxysubsection{Why aren\textquotesingle{}t Google Test assertions implemented using exceptions?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md749}
...
The downside of not using exceptions is that {\ttfamily A\+S\+S\+E\+R\+T\+\_\+$\ast$} (implemented using {\ttfamily return}) will only abort the current function, not the current {\ttfamily T\+E\+ST}.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md750}{}\doxysubsection{Why do we use two different macros for tests with and without fixtures?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md750}
...
We think neither approach is ideal, yet either of them is reasonable. In the end, it probably doesn\textquotesingle{}t matter much either way.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md751}{}\doxysubsection{Why don\textquotesingle{}t we use structs as test fixtures?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md751}
We like to use structs only when representing passive data. This distinction between structs and classes is good for documenting the intent of the code\textquotesingle{}s author. Since test fixtures have logic like {\ttfamily Set\+Up()} and {\ttfamily Tear\+Down()}, they are better defined as classes.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md752}{}\doxysubsection{Why are death tests implemented as assertions instead of using a test runner?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md752}
...
Otherwise your code is {\bfseries{invalid C++}}, and may break in unexpected ways. In particular, using it in Google Test comparison assertions ({\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+EQ}, etc) will generate an \char`\"{}undefined reference\char`\"{} linker error.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md755}{}\doxysubsection{I have an interface that has several implementations. Can I write a set of tests once and repeat them over all the implementations?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md755}
Google Test doesn\textquotesingle{}t yet have good support for this kind of tests, or data-\/driven tests in general. We hope to be able to make improvements in this area soon.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md756}{}\doxysubsection{Can I derive a test fixture from another?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md756}
Yes.
...
For a complete example using derived test fixtures, see \href{../samples/sample5_unittest.cc}{\texttt{ sample5}}.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md757}{}\doxysubsection{My compiler complains \char`\"{}void value not ignored as it ought to be.\char`\"{} What does this mean?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md757}
You\textquotesingle{}re probably using an {\ttfamily A\+S\+S\+E\+R\+T\+\_\+$\ast$()} in a function that doesn\textquotesingle{}t return {\ttfamily void}. {\ttfamily A\+S\+S\+E\+R\+T\+\_\+$\ast$()} can only be used in {\ttfamily void} functions.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md758}{}\doxysubsection{My death test hangs (or seg-\/faults). How do I fix it?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md758}
...
\end{DoxyItemize}\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md760}{}\doxysubsection{The compiler complains \char`\"{}no matching function to call\char`\"{} when I use A\+S\+S\+E\+R\+T\+\_\+\+P\+R\+E\+Dn. How do I fix it?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md760}
...
\DoxyCodeLine{ASSERT\_PRED2(*(GreaterThan<int, int>)*, 5, 0);}
\end{DoxyCode}
\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md761}{}\doxysubsection{My compiler complains about \char`\"{}ignoring return value\char`\"{} when I call R\+U\+N\+\_\+\+A\+L\+L\+\_\+\+T\+E\+S\+T\+S(). Why?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md761}
...
To help the users avoid this dangerous bug, the implementation of {\ttfamily R\+U\+N\+\_\+\+A\+L\+L\+\_\+\+T\+E\+S\+T\+S()} causes gcc to raise this warning, when the return value is ignored. If you see this warning, the fix is simple\+: just make sure its value is used as the return value of {\ttfamily main()}.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md762}{}\doxysubsection{My compiler complains that a constructor (or destructor) cannot return a value. What\textquotesingle{}s going on?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md762}
...
we had to give up using {\ttfamily A\+S\+S\+E\+R\+T$\ast$} and {\ttfamily F\+A\+I\+L$\ast$} (but not {\ttfamily E\+X\+P\+E\+C\+T$\ast$} and {\ttfamily A\+D\+D\+\_\+\+F\+A\+I\+L\+U\+R\+E$\ast$}) in constructors and destructors. The workaround is to move the content of your constructor/destructor to a private void member function, or switch to {\ttfamily E\+X\+P\+E\+C\+T\+\_\+$\ast$()} if that works. This section in the user\textquotesingle{}s guide explains it.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md763}{}\doxysubsection{My set-\/up function is not called. Why?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md763}
C++ is case-\/sensitive. It should be spelled as {\ttfamily Set\+Up()}. Did you spell it as {\ttfamily Setup()}?

Similarly, sometimes people spell {\ttfamily Set\+Up\+Test\+Case()} as {\ttfamily Setup\+Test\+Case()} and wonder why it\textquotesingle{}s never called.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md764}{}\doxysubsection{How do I jump to the line of a failure in Emacs directly?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md764}
Google Test\textquotesingle{}s failure message format is understood by Emacs and many other I\+D\+Es, like acme and X\+Code. If a Google Test message is in a compilation buffer in Emacs, then it\textquotesingle{}s clickable. You can now hit {\ttfamily enter} on a message to jump to the corresponding source code, or use {\ttfamily C-\/x}\`{} to jump to the next failure.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md765}{}\doxysubsection{I have several test cases which share the same test fixture logic, do I have to define a new test fixture class for each of them? This seems pretty tedious.}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md765}
...
\DoxyCodeLine{TEST\_F(BarTest, Def) \{ ... \}}
\end{DoxyCode}
\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md766}{}\doxysubsection{The Google Test output is buried in a whole bunch of log messages. What do I do?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md766}
...
\end{DoxyCode}
\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md767}{}\doxysubsection{Why should I prefer test fixtures over global variables?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md767}
...
\end{DoxyEnumerate}\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md768}{}\doxysubsection{How do I test private class members without writing F\+R\+I\+E\+N\+D\+\_\+\+T\+E\+S\+T()s?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md768}
...
\end{DoxyItemize}\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md769}{}\doxysubsection{How do I test private class static members without writing F\+R\+I\+E\+N\+D\+\_\+\+T\+E\+S\+T()s?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md769}
...
\end{DoxyCode}
\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md770}{}\doxysubsection{I would like to run a test several times with different parameters. Do I need to write several similar copies of it?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md770}
No. You can use a feature called \href{AdvancedGuide.md\#Value_Parameterized_Tests}{\texttt{ value-\/parameterized tests}} which lets you repeat your tests with different parameters, without defining it more than once.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md771}{}\doxysubsection{How do I test a file that defines main()?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md771}
...
However, please remember this is a hack and should only be used as the last resort.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md772}{}\doxysubsection{What can the statement argument in A\+S\+S\+E\+R\+T\+\_\+\+D\+E\+A\+T\+H() be?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md772}
...
{\ttfamily googletest\+\_\+unittest.\+cc} contains more examples if you are interested.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md773}{}\doxysubsection{What syntax does the regular expression in A\+S\+S\+E\+R\+T\+\_\+\+D\+E\+A\+T\+H use?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md773}
On P\+O\+S\+IX systems, Google Test uses the P\+O\+S\+IX Extended regular expression syntax (\href{http://en.wikipedia.org/wiki/Regular_expression\#POSIX_Extended_Regular_Expressions}{\texttt{ http\+://en.\+wikipedia.\+org/wiki/\+Regular\+\_\+expression\#\+P\+O\+S\+I\+X\+\_\+\+Extended\+\_\+\+Regular\+\_\+\+Expressions}}). On Windows, it uses a limited variant of regular expression syntax. For more details, see the \href{AdvancedGuide.md\#Regular_Expression_Syntax}{\texttt{ regular expression syntax}}.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md774}{}\doxysubsection{I have a fixture class Foo, but T\+E\+S\+T\+\_\+\+F(\+Foo, Bar) gives me error \char`\"{}no matching function for call to Foo\+::\+Foo()\char`\"{}. Why?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md774}
...
\end{DoxyItemize}\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md775}{}\doxysubsection{Why does A\+S\+S\+E\+R\+T\+\_\+\+D\+E\+A\+T\+H complain about previous threads that were already joined?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md775}
...
The new N\+P\+TL thread library doesn\textquotesingle{}t suffer from this problem, as it doesn\textquotesingle{}t create a manager thread. However, if you don\textquotesingle{}t control which machine your test runs on, you shouldn\textquotesingle{}t depend on this.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md776}{}\doxysubsection{Why does Google Test require the entire test case, instead of individual tests, to be named F\+O\+O\+Death\+Test when it uses A\+S\+S\+E\+R\+T\+\_\+\+D\+E\+A\+T\+H?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md776}
...
\end{DoxyCode}
\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md778}{}\doxysubsection{The compiler complains about \char`\"{}no match for \textquotesingle{}operator$<$$<$\textquotesingle{}\char`\"{} when I use an assertion. What gives?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md778}
...
In addition, if {\ttfamily Foo\+Type} is declared in a name space, the {\ttfamily $<$$<$} operator also needs to be defined in the {\itshape same} name space.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md779}{}\doxysubsection{How do I suppress the memory leak messages on Windows?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md779}
Since the statically initialized Google Test singleton requires allocations on the heap, the Visual C++ memory leak detector will report memory leaks at the end of the program run. The easiest way to avoid this is to use the {\ttfamily \+\_\+\+Crt\+Mem\+Checkpoint} and {\ttfamily \+\_\+\+Crt\+Mem\+Dump\+All\+Objects\+Since} calls to not report any statically initialized heap objects. See M\+S\+DN for more details and additional heap check/debug routines.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md780}{}\doxysubsection{I am building my project with Google Test in Visual Studio and all I\textquotesingle{}m getting is a bunch of linker errors (or warnings). Help!}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md780}
...
To update this setting open the project properties in the Visual Studio I\+DE then select the branch Configuration Properties $\vert$ C/\+C++ $\vert$ Code Generation and change the option \char`\"{}\+Runtime Library\char`\"{}. You may also try using gtest-\/md.\+vcproj instead of gtest.\+vcproj.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md781}{}\doxysubsection{I put my tests in a library and Google Test doesn\textquotesingle{}t run them. What\textquotesingle{}s happening?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md781}
Have you read a \href{Primer.md\#important-note-for-visual-c-users}{\texttt{ warning}} on the Google Test Primer page?\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md782}{}\doxysubsection{I want to use Google Test with Visual Studio but don\textquotesingle{}t know where to start.}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md782}
Many people are in your position and one of the posted his solution to our mailing list.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md783}{}\doxysubsection{I am seeing compile errors mentioning std\+::type\+\_\+traits when I try to use Google Test on Solaris.}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md783}
...
`export CC=cc C\+XX=CC C\+X\+X\+F\+L\+A\+GS='-\/library=stlport4\textquotesingle{}\`{}\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md784}{}\doxysubsection{How can my code detect if it is running in a test?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md784}
...
However, if you {\itshape really}, {\itshape really}, {\itshape really} have no choice, and if you follow the rule of ending your test program names with {\ttfamily \+\_\+test}, you can use the {\itshape horrible} hack of sniffing your executable name ({\ttfamily argv\mbox{[}0\mbox{]}} in {\ttfamily main()}) to know whether the code is under test.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md785}{}\doxysubsection{Google Test defines a macro that clashes with one defined by another library. How do I deal with that?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md785}
...
Currently, the following {\ttfamily T\+E\+ST}, {\ttfamily F\+A\+IL}, {\ttfamily S\+U\+C\+C\+E\+ED}, and the basic comparison assertion macros can have alternative names. You can see the full list of covered macros \href{http://www.google.com/codesearch?q=if+!GTEST_DONT_DEFINE_\\w\%2B+package:http://googletest\\.googlecode\\.com+file:/include/gtest/gtest.h}{\texttt{ here}}. More information can be found in the \char`\"{}\+Avoiding Macro Name Clashes\char`\"{} section of the R\+E\+A\+D\+ME file.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md786}{}\doxysubsection{Is it O\+K if I have two separate $<$tt$>$\+T\+E\+S\+T(\+Foo, Bar)$<$/tt$>$ test methods defined in different namespaces?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md786}
Yes.
...
\DoxyCodeLine{\}  // namespace foo}
\end{DoxyCode}
\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md787}{}\doxysubsection{How do I build Google Testing Framework with Xcode 4?}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md787}
If you try to build Google Test\textquotesingle{}s Xcode project with Xcode 4.\+0 or later, you may encounter an error message that looks like \char`\"{}\+Missing S\+D\+K in target gtest\textbackslash{}\+\_\+framework\+: /\+Developer/\+S\+D\+Ks/\+Mac\+O\+S\+X10.\+4u.\+sdk\char`\"{}. That means that Xcode does not support the S\+DK the project is targeting. See the Xcode section in the ../\+R\+E\+A\+D\+ME.md \char`\"{}\+R\+E\+A\+D\+M\+E\char`\"{} file on how to resolve this.\hypertarget{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md788}{}\doxysubsection{My question is not covered in your F\+A\+Q!}\label{md_vendor_googletest_googletest_docs__f_a_q_autotoc_md788}
