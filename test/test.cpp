...
TEST(Yolo_constuctor_test, yolo) {
  Yolo yol("../models/yolov5n.onnx");
  EXPECT_EQ(yol.model_path, "../models/yolov5n.onnx");
}

TEST(Image_constuctor_test, image) {
  cv::Mat im = cv::imread("../segmentations/coco_names.txt", 1);
  Image img(im);
}
...
TEST(Image_draw_rectangles_Test, draw_rectangles) {
  ...
  cv::Mat im = cv::imread("../videos/shiva.jpg", 1);
  Image img(im);
  EXPECT_EQ(img.get_image().cols, img.draw_rectangles(yol.output).cols);
}
...
TEST(Image_rectangle_draw, Test2) {
  ...
  cv::Mat im = cv::imread("../videos/shiva.jpg", 1);
  Image img(im);
  auto img2 = img.draw_rectangles(yol.output);
  if (yol.output.size() == 0) {
    EXPECT_FALSE(cv::norm(im, img2, cv::NORM_L1));
  } else {
    EXPECT_TRUE(cv::norm(im, img2, cv::NORM_L1));
  }
}
